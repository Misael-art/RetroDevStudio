...
**Nota 2026-07-07 (rodada Node-03, branch `codex/node-engine-local-mvp`, stacked sobre `codex/node-engine-contract-hardening-clean`/Linux):** Node Engine **local MVP** fechado, ainda **Experimental**: modelo de dados e serializacao v1 movidos da UI para o core (`src/core/nodegraph/nodeTypes.ts` + `nodeDefinitions.ts`; `NodeGraphEditor.tsx` e apresentacao e re-exporta por compatibilidade), Inspector consumindo `LocalNodeTrace` real de `runNodeGraphLocally` (sucesso = trace tipado; falha = erros estruturados "Execucao local bloqueada") com rotulos "Local trace"/"simulado / nao instrumentado (Experimental)" e linha explicita `Runtime: nao suportado` via `resolveRuntimeEvidenceForGraph()`, e `nodeCompiler.ts` convergido aos tipos do core e isolado como legado com guard de teste que proibe import de producao. Segue NAO implementado: runtime real, source mapping ROM/PC -> node, evidencia de emulador e integracao build/runtime real. Detalhe em `docs/06_CURRENT_WAVE_AI_BANK.md` rodada Node-03.

**Nota 2026-07-06 (rodadas Node-01/Node-02, branch `codex/node-engine-contract-hardening-clean`, stacked sobre `codex/linux-host-support`):** o contrato minimo do Node Engine foi extraido do componente UI para o core em `src/core/nodegraph/nodeEngine.ts`: validador canonico `validateNodeGraph` movido para o core (editor re-exporta; sem fluxo paralelo), erros estruturados (`graph_empty`, node/porta inexistente, tipo incompativel, ciclo exec), executor local puro/deterministico (`runNodeGraphLocally`) e separacao explicita de evidencia `LocalNodeTrace` (sempre `simulated`) vs `RuntimeEvidence` (reservado, nunca fabricado) vs `UnsupportedRuntimeMapping` (estado atual: sem source mapping ROM/PC -> node). O trace do NodeGraph Inspector continua **local/simulado/Experimental** ("simulado / nao instrumentado (Experimental)"); nada foi conectado a runtime real e nenhuma superficie foi promovida. Cobertura: +12 testes unitarios do contrato (`nodeEngine.test.ts`). Base recomendada do PR: `main` apos o merge do suporte Linux, ou `codex/linux-host-support` como stack temporario. Detalhe em `docs/06_CURRENT_WAVE_AI_BANK.md` rodadas Node-01/Node-02.

**Nota 2026-06-17 (rodada 71, branch `codex/w7-3-parity-robustness`, commit `e8f5e82`):** W7.3 finalizada. Lacunas da rodada 70 corrigidas: check-tree estrito restaurado (`.serena` nao mais ignorado; diretorio movido para backup fora da raiz), `ProjectCapabilityPanel` nao chama mais parity com golden vazio (orienta para a aba canonica **Tools -> Parity Capture**; `RuntimeConsolePanel` nao existe na arvore atual, ToolsPanel e a superficie canonica), cobertura de UI parity ampliada e store `lastParityReport` atualizado apos captura OK. Causa raiz da falha real (Rust + QA-RC Build & Run) corrigida sem mascarar: SGDK em path com espaco (`F:\Projects\Sgdk Forge`) quebrava o GNU make (`*** empty variable name. Stop.`); `build_orch.rs` agora passa `SGDK`/`GDK` ao make como caminho sem espacos (8.3 short name ou junction `mklink /J`), com regressao `megadrive_build_passes_space_free_sgdk_paths_to_make`. Gates no commit: check:tree, lint, tsc, **411 npm test**, clippy, **436/0/23 cargo test --lib**, preflight **Ready: SIM**, **qa-rc A-H passed**, `release:readiness:promotion` com unico bloqueador = governanca de branch (2 a frente de `origin/main`), apontando para `e8f5e82`. Artefatos frescos 2026-06-17: EXE portable (23.555.584 B), MSI (8.249.344 B) e `release-manifest.json`. Gameplay Parity continua **Experimental**; nada promovido para Stable. Limites honestos: nao e BlastEm, nao faz cycle accuracy, nao prova gameplay 1:1. Proximas frentes nao iniciadas: W7.4 (BlastEm) e W7.5 (cycle report).
...
**Nota 2026-05-27 (rodada 64, branch `codex/asset-browser-production-x-rebased`):** Asset Browser X foi reconstruido de forma conservadora sobre `origin/main` `5090d5957ceddcfbe9ac730391479616ce357cd6`, preservando o worktree antigo sujo e backups antes de qualquer limpeza. A fatia reaproveitada adiciona busca/filtros por tipo/gerado/orfao/over-budget, classificacao de assets, resumo rapido de budget com `HwStatus`, preview seguro de nao-imagens, "Usado por" com cena/grafo, insercao real na Scene e ponte direta para o ArtStudio. Foram descartados como superseded/ruido os commits antigos ja absorvidos por `main` e schemas/gerados Tauri sem delta material. Gates frescos: `check:tree`, `lint`, `tsc`, `npm test` **386/386**, `cargo clippy`, `cargo test --lib` **413 passed / 23 ignored**, `test:e2e:desktop:qa-rc` A-H (`qa-rc-2026-05-27T09-49-29-979Z-*`) e `git diff --check`. Asset Browser permanece **Experimental/em hardening**; isto nao promove ArtStudio, SGDK, Node Engine, SNES, GameMaker, Godot, MUGEN/Ikemen, OpenBOR ou AAA para Stable.

**Nota 2026-05-26 (rodada 62, branch `codex/command-palette-shortcut-editor-r`):** PR #15 foi rebaseado sobre `origin/main` (`42db9fb30dcc79d659929c6f0d285f2be449d325`) apos Godot Y entrar em `main`; o conflito documental em `docs/03_ROADMAP_MVP.md` foi resolvido preservando as notas de `main` e a nota especifica da Command Palette R. O review encontrou um bug real: o destaque visual indexava `results`, enquanto Enter executava `activeResults`, podendo executar item diferente quando comandos disabled vinham no topo. A correcao mantem `selectedIndex` indexando apenas `activeResults`, renderiza o destaque pelo indice ativo correspondente, impede execucao quando nao ha comando ativo e alinha Enter, ArrowUp/ArrowDown, hover/focus e click. Regressao adicionada em `src/App.test.tsx` cobre disabled no topo, Enter alinhado ao destaque, setas, hover/click, lista sem item ativo e filtro que reduz resultados. Gates frescos: `check:tree`, `lint`, `tsc`, foco CommandPaletteDialog **5 passed / 61 skipped**, `npm test` **379/379**, `cargo clippy`, `cargo test --lib` **413 passed / 23 ignored**, `git diff --check` e QA RC A-H (`qa-rc-2026-05-26T23-54-07-269Z-*`) com `RDS_EDGE_DRIVER_PATH` apontando para o driver local canonico existente. Isto e UX/CX em hardening; nao promove SGDK/Node Engine/ArtStudio/SNES/GameMaker/Godot/MUGEN/OpenBOR/AAA para Stable.
...
**Nota 2026-05-23 (Agente Q, branch `codex/aaa-capability-layer-q`):** adicionada uma camada transversal de capability diagnostics para evidenciar, sem marketing falso, os eixos documentacao, implementacao, build, ROM, emulacao, runtime evidence, validacao visual, assets, patterns, runtime contracts e audio. O modelo `ProjectCapabilityReport` e os comandos Tauri novos sao operacionais, mas todos os eixos permanecem **Experimental** ate haver build real, ROM real, emulacao real e evidencia institucional fresca. Integracoes: painel de capability no Debug/Tools, status de ROM mastering no fluxo Build/Game, contratos runtime no Inspector, `Qualidade ROM` no ArtStudio e galeria SGDK experimental no NodeGraph. Gates locais desta branch fecharam `check:tree`, `lint`, `tsc`, `npm test`, `cargo clippy`, `cargo test --lib`, `build:debug`, preflight SGDK, QA-RC desktop A-H e upstream Windows. Isto nao altera a fase do roadmap, nao promove SGDK/Node Engine/ArtStudio/SNES/importadores e nao cria tela de vitrine.
...
**Nota 2026-05-20 (rodada 49, branch `codex/sgdk-real-proof-integration`):** SGDK Logic Extractor v1 foi incorporado ao inventario canonico, nao como formato paralelo. `canonical_model.logic_systems` passa a expor funcoes, chamadas, FSMs, estados, transicoes, condicoes, acoes, refs de entidades/pools, source mappings, semantic gaps/bridges e confidence para os geradores de nodes. Fixtures cobrem enum/switch, `#define STATE_*`, input SGDK, animacao, transicoes diretas, macro complexa como bridge e padrao BLAZE-like. Relatorios host-local de Platformer 2, NEXZR MD e BLAZE_ENGINE foram gerados em `src-tauri/target-test/validation/sgdk-logic-extractor-v1/`, todos com `blocking_gaps=0`. A matriz SGDK real segue verde **7/7** quando `SGDK_ROOT` e o core Genesis Plus GX reais sao explicitados no host. Isto fortalece a ponte SGDK -> modelo intermediario, mas nao altera por si so badges/UI nem declara nova promocao de NodeGraph visual.
**Ultima revisao canonica:** 2026-05-20 (rodada 49)
**Fase ativa real:** Core MVP promovido tecnicamente em main; hardening continua nas superficies experimentais
...
**Nota 2026-05-20 (rodada 49, branch `codex/sgdk-logic-truth-product-qa`):** SGDK Logic ganhou uma camada de produto/QA para mostrar suporte real sem inflar maturidade: matriz de capacidades SGDK, resumo pos-import com contadores e source files, sinais compactos em Hierarchy/Inspector e NodeGraph com badges de conversao/bridge/gap/source mapping. O `qa-rc` bloco G agora captura `G-scene-import-summary`, `G-logic-fsm-graph`, `G-node-source-mapping`, `G-gap-bridge-panel` e `G-sgdk-chain`, com asserts de NodeGraph visivel, nodes sem sobreposicao grosseira, Source Mapping, Import Gaps e shell sem overlap textual. Evidencia fresca: `qa-rc-2026-05-20T16-35-10-947Z-*`, corpus real **122/122** processados (**68** build/ROM/emulacao visivel, **54** bridge-only) e matriz SGDK **7/7**. Status: Build/ROM/Emulacao SGDK preservam certificacao existente; Logica por nodes e FSM importado seguem **Parcial/Experimental** ate A+B+C passarem integrados; equivalencia gameplay nao certificada sem harness especifico.
...
**Nota 2026-05-19 (rodada 47, branch `codex/compatibility-harness-gamemaker-vertical`, em main):** prova vertical GameMaker real -> nodes (subset `oPlayer`) -> SGDK C -> ROM -> Libretro visivel para `Basic_platform_game_example.gmez`. GameMaker **Experimental/importavel**; GML completo e `.yyp/.yy` pendentes.

**Nota 2026-05-19 (rodada 47, branch `codex/command-dat-artstudio-node-runtime`):** `command.dat` -> comandos visuais -> nodes executaveis; ArtStudio painel `Comandos`; node `input_command`; codegen SGDK/SNES com ring buffer. Status **Experimental**; sem compatibilidade total com todo dialeto.
**Ultima revisao canonica:** 2026-05-20 (rodada 48)
**Fase ativa real:** Core MVP promovido tecnicamente em main; hardening continua nas superficies experimentais

**Nota 2026-05-20 (rodada 48, branch `codex/gamemaker-modern-gml-runtime-d`):** GameMaker avanca para subset amplo funcional/auditavel sem sair de **Experimental/importavel**. O adapter passa a detectar `.gmx`, `.gmz`, `.gmez`, `.yyp` e `.yy`; importa sprites, objects, rooms, instances, views/cameras, tile layers, collision masks e eventos Create/Step/Draw/Collision/Alarm; e amplia GML -> nodes para input, movimento, pulo/gravidade, colisao com `place_free`/`place_meeting`, `sprite_index`, `image_xscale`, `image_speed`, `alarm[]`, `instance_create`, `instance_destroy`, transicao simples de room e variaveis. Scripts complexos, shaders, surfaces, ds_* complexos, physics Box2D, particulas e extensoes nativas seguem como bridges estruturadas com fonte/linha/motivo/impacto/sugestao. Isto nao declara GML completo, paridade 1:1 nem Stable geral.

**Nota 2026-05-19 (rodada 47, branch `codex/compatibility-harness-gamemaker-vertical`):** fechada prova vertical host-local GameMaker real -> nodes oficiais (subset `oPlayer`) -> SGDK C -> ROM Mega Drive -> Libretro visivel para `F:\Projects\Game Maker\Basic_platform_game_example.gmez`, via `compatibility_harness.rs` + `gml_to_nodes.rs`. Evidencia: `cargo test --lib gamemaker_vertical_compatibility_harness_basic_platform -- --ignored --nocapture --test-threads=1` com `non_black_pixels=3793`, `fake_toolchain_used=false`, relatorio em `src-tauri/target-test/validation/gamemaker-vertical/`. GameMaker permanece **Experimental/importavel** (nao Stable geral); GML completo, `.yyp/.yy` modernos e paridade visual total do template seguem pendentes.

**Nota 2026-05-18 (rodada 46, branch `codex/product-compatibility-wave`):** iniciada uma fatia conservadora de compatibilidade real GameMaker sem promover maturidade. O perfil `gamemaker` saiu de Parcial/nao importavel para **Experimental/importavel** com adapter GMX/GMZ/GMEZ: extrai pacotes 7z via dependencia ja existente `sevenz-rust2`, detecta layouts `sprites/objects/rooms` e `Assets/Sprites/Objects/Rooms`, importa sprites/objetos/rooms, cria entidades editaveis, preserva GML de eventos/instances como bridge semantica visivel (`logic_hints`, `graph_origin=gamemaker_gmx`, `imported_semantics.source=gamemaker_gmx`) e adiciona camera a partir de views. Evidencias focadas: `cargo test --manifest-path src-tauri/Cargo.toml --lib import_gamemaker_gmx_project_creates_editable_scene_with_preserved_gml_bridge -- --nocapture --test-threads=1`, `cargo test --manifest-path src-tauri/Cargo.toml --lib list_external_import_profiles_exposes_support_matrix -- --nocapture --test-threads=1` e host-local `cargo test --manifest-path src-tauri/Cargo.toml --lib import_gamemaker_real_host_samples_when_present -- --ignored --nocapture --test-threads=1` contra `F:\Projects\Game Maker\Basic_platform_game_example.gmez` e `F:\Projects\Godot\Super Bowsette Source Code Beta 0.1\src`. Isto **nao** declara conversao GML nativa completa, ROM/emulacao GameMaker institucional nem Stable; build/ROM/emulacao E2E dedicado ainda e bloqueador para subir maturidade.
...
| Logic workspace / NodeGraph canonico   | Core MVP     | Em codigo     | Em hardening  | `NodeGraphEditor.test.tsx`, `nodeCompiler.test.ts`, emissao SGDK/SNES reais, prova `qa-rc` rodada 29 `G-logic-authoring`, validador rodada 36 e rodada 49 com badges SGDK import (`Converted`/`Bridge`/`Gap`/`Source mapped`), Source Mapping e Import Gaps | Refinamento continuo de UX do canvas; sem vender heuristica Phase D como AST/FSM completo | Nao                           |
...
| Asset Browser                          | Experimental | Em codigo     | Em hardening  | Rodada 65 em `main`/PR #17: busca/filtros por tipo/gerado/orfao/over-budget, preview seguro, "Usado por" com cena/grafo, budget rapido, insercao na Scene, ponte ArtStudio, readiness verde, builds finais e checks remotos `CI`/`Desktop E2E` success; rotulo `experimental` permanece alinhado ao roadmap | Manter rotulo e repetir QA institucional quando o fluxo de assets mudar; ainda falta polish/prova de produto dedicada antes de qualquer subida de maturidade | Nao                           |
...
| `mugen`          | Experimental   | Em codigo     | Local        | `import_mugen_project`, wizard dedicado, smoke idempotente e subset de luta 2D da rodada 51: AIR/CLSN, CMD->`input_command`, CNS/ST->FSM/velocidade/posicao/som, `HitDef` como bridge, stage tilemap/parallax.                         | Falta prova institucional ampla, ROM/Libretro para samples reais variados e cobertura de dialetos/comportamentos MUGEN fora do subset | Nao                           |
| `ikemen_go`      | Experimental   | Em codigo     | Local        | Perfil proprio no registry, roteado pelo adapter MUGEN com `source_engine=ikemen_go`; rodada 51 preserva perfil e transforma extensoes comuns/nao suportadas em bridges explicitas sem perder assets.                                  | Falta evidencia institucional dedicada, corpus Ikemen robusto e validacao de compatibilidade alem do subset MUGEN compartilhado | Nao                           |
| `sgdk`           | Experimental   | Em codigo     | Em hardening | Fase E provada: desktop E2E `qa-rc` A-G verde (import -> colisao -> persistir -> reabrir -> Build & Run -> ROM `SEGA` verificada). Rodada 49 adiciona matriz de capacidades, resumo pos-import SGDK Logic, badges NodeGraph, Source Mapping e Import Gaps filtravel para separar nodes funcionais, bridges e gaps. Build/ROM/Emulacao mantem prova real de subset; BLAZE segue coberto por modo compativel/bridge quando necessario. | Logica por nodes/FSM importado permanece **Parcial/Experimental** ate Semantic Extractor + NodeGraph/codegen + UX/QA passarem integrados; equivalencia gameplay nao certificada sem harness; criterio explicito para sair de **Experimental** continua em `docs/SGDK_REAL_CORPUS_VALIDATION_MATRIX.md` | Nao                           |
...
| `gamemaker`      | Fora do MVP/Q2 | Experimental  | Local        | Registry `importable: true`; adapter GMX/GMZ/GMEZ/YYP/YY importa rooms/instances/sprites/objects/views/tile layers/collision masks/eventos, agrega paredes em `collision_map`, converte subset GML ampliado para nodes oficiais (`gml_to_nodes.rs`) e expoe harness vertical (`compatibility_harness.rs`). Prova host-local: `Basic_platform_game_example.gmez` -> ROM -> Libretro (`non_black_pixels>0`, relatorio em `target-test/validation/gamemaker-vertical/`). | Falta GML completo, CI institucional do harness, sprites MD para todos os props, paridade gameplay 1:1 e promocao governada em destino canonico. | Nao                           |
| `openbor`        | Experimental   | Em codigo     | Local        | Subset beat'em up funcional: `models.txt`/`levels.txt`, chars e levels com `anim`, `offset`, `bbox`, `attack`, `delay`, `loop`, `jumpframe`, `landframe`, `spawn`, `at`, `music`, `panel`, `background` e scroll; conversao para animacoes, hitboxes/collision metadata, timeline `spawn_entity`, nodes `set_velocity`/`set_animation_state`/`scroll_tilemap`/`move_camera` e bridges para scripts nao suportados. Smoke idempotente permanece em `smoke_import_openbor_project_builds_scene_and_is_idempotent`. | Falta prova institucional dedicada com projetos OpenBOR reais/BYOR, validacao manual ampla dos dialetos e evidencia repetida de SGDK C/ROM/Libretro para amostras oficiais; subset nao e compatibilidade total de OpenBOR | Nao                           |
| `gamemaker`      | Fora do MVP/Q2 | Experimental  | Local        | Registry `importable: true`; adapter GMX/GMZ/GMEZ importa rooms/instances/sprites, agrega paredes em `collision_map`, converte subset GML do player para nodes oficiais (`gml_to_nodes.rs`) e expoe harness vertical (`compatibility_harness.rs`). Prova host-local: `Basic_platform_game_example.gmez` -> ROM -> Libretro (`non_black_pixels>0`, relatorio em `target-test/validation/gamemaker-vertical/`). | Falta GML completo, `.yyp/.yy` modernos, CI institucional do harness, sprites MD para todos os props e paridade gameplay 1:1. | Nao                           |
