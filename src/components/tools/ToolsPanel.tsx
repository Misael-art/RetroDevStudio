...
function ReverseWorkspace() {
  const { activeTarget, logMessage } = useEditorStore();
  const [romPath, setRomPath] = useState("");
  const [activeView, setActiveView] = useState<
    "map" | "hex" | "graphics" | "text" | "audio" | "code" | "projection"
  >("map");
  const [busy, setBusy] = useState(false);
  ...
  const [codeResult, setCodeResult] = useState<Awaited<ReturnType<typeof romDisassemble>> | null>(null);
  ...
  async function inspectHex(nextManifest?: RomAnalysisManifest | null, nextOffsetHex?: string) {
    ...
    const target =
      activeManifest.target === "snes" ? "snes" : ("megadrive" as "megadrive" | "snes");
      ...
      {manifest && (
        <>
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            <div className="rounded border border-[#313244] bg-[#11111b] p-3">
              <div className="text-[10px] uppercase tracking-[0.16em] text-[#7f849c]">Formato</div>
            ...
            <div className="rounded border border-[#313244] bg-[#11111b] p-3">
              <div className="text-[10px] uppercase tracking-[0.16em] text-[#7f849c]">Identidade</div>
              <div className="mt-2 text-sm font-semibold text-[#e5e7eb]">
            ...
            <div className="rounded border border-[#313244] bg-[#11111b] p-3">
              <div className="text-[10px] uppercase tracking-[0.16em] text-[#7f849c]">Hashes</div>
              <div className="mt-2 text-[10px] font-mono text-[#cdd6f4]">CRC32 {manifest.hashes.crc32}</div>
              <div className="mt-1 text-[10px] font-mono text-[#94a3b8]">{manifest.hashes.sha1}</div>
            </div>
            <div className="rounded border border-[#313244] bg-[#11111b] p-3">
              <div className="text-[10px] uppercase tracking-[0.16em] text-[#7f849c]">Sinais</div>
              <div className="mt-2 text-[10px] text-[#94a3b8]">
                GFX {manifest.graphics_regions.length} · TXT {manifest.text_regions.length} · AUD {manifest.audio_regions.length}
              </div>
              ...
              <div className="space-y-2">
                {manifest.segments.slice(0, 24).map((segment) => {
                  const widthPct = Math.max(
                    3,
                    Math.round(((segment.end - segment.start) / Math.max(manifest.total_size, 1)) * 100)
                  );
                  return (
          ...
          {activeView === "graphics" && (
            <div className="space-y-2 rounded border border-[#313244] bg-[#11111b] p-3">
              {manifest.graphics_regions.length === 0 ? (
                <p className="text-[10px] text-[#7f849c]">Nenhuma regiao grafica candidata detectada.</p>
              ) : (
                manifest.graphics_regions.map((candidate) => (
                  <div key={candidate.id} className="rounded border border-[#1e1e2e] bg-[#0f172a] p-3">
                    ...
                    <div className="mt-2 text-[10px] text-[#94a3b8]">
                      {candidate.bpp}bpp · {candidate.tile_count} tiles · paleta {candidate.palette_slot ?? "?"} · conf. {candidate.confidence}%
                    </div>
          ...
          {activeView === "text" && (
            <div className="grid gap-3 xl:grid-cols-[minmax(0,2fr)_minmax(280px,1fr)]">
              <div className="space-y-2 rounded border border-[#313244] bg-[#11111b] p-3">
                <div className="text-[10px] uppercase tracking-[0.16em] text-[#7f849c]">Strings</div>
                {manifest.text_regions.length === 0 ? (
                  <p className="text-[10px] text-[#7f849c]">Nenhuma string candidata detectada.</p>
                ) : (
              ...
              <div className="space-y-2 rounded border border-[#313244] bg-[#11111b] p-3">
                <div className="text-[10px] uppercase tracking-[0.16em] text-[#7f849c]">Pointer tables</div>
                {manifest.pointer_tables.length === 0 ? (
                  <p className="text-[10px] text-[#7f849c]">Nenhuma pointer table candidata detectada.</p>
                ) : (
                  manifest.pointer_tables.map((table) => (
                    <div key={`${table.start}-${table.end}`} className="rounded border border-[#1e1e2e] bg-[#0f172a] p-3 text-[10px]">
                      ...
                      <div className="mt-1 text-[#94a3b8]">
                        {table.encoding} · entrada {table.entry_size} byte(s) · conf. {table.confidence}%
                      </div>
          ...
          {activeView === "audio" && (
            <div className="space-y-2 rounded border border-[#313244] bg-[#11111b] p-3">
              {manifest.audio_regions.length === 0 ? (
                <p className="text-[10px] text-[#7f849c]">Nenhum candidato de audio detectado nesta ROM.</p>
              ) : (
            ...
            <div className="grid gap-3 xl:grid-cols-[minmax(280px,360px)_minmax(0,1fr)]">
              <div className="space-y-2 rounded border border-[#313244] bg-[#11111b] p-3">
                ...
                {prioritizedFunctions.length ? (
                  ...
                  <p className="text-[10px] text-[#7f849c]">Nenhuma funcao candidata mapeada.</p>
                )}
                ...
                <div className="space-y-2 pt-2">
                  <div className="text-[10px] uppercase tracking-[0.16em] text-[#7f849c]">Anotacoes</div>
                  ...
                  {manifest.annotations.length === 0 ? (
                    <p className="text-[10px] text-[#7f849c]">Nenhuma anotacao persistida para esta ROM.</p>
                  ) : (
                    manifest.annotations.slice(0, 12).map((annotation, index) => (
                      <div key={`${annotation.kind}-${annotation.start}-${annotation.end ?? index}`} className="rounded border border-[#1e1e2e] bg-[#0f172a] p-3 text-[10px]">
                        <div className="flex items-center justify-between gap-3">
          ...
          {activeView === "projection" && (
            <div className="space-y-3 rounded border border-[#313244] bg-[#11111b] p-3">
              ...
              <div className="grid gap-3 xl:grid-cols-2">
                <div className="rounded border border-[#1e1e2e] bg-[#0f172a] p-3">
                  <div className="text-[10px] uppercase tracking-[0.16em] text-[#7f849c]">Projection status</div>
                  <div className="mt-2 text-sm font-semibold text-[#e5e7eb]">
                ...
                <div className="rounded border border-[#1e1e2e] bg-[#0f172a] p-3">
                  <div className="text-[10px] uppercase tracking-[0.16em] text-[#7f849c]">Logic hints</div>
                  <div className="mt-2 text-[10px] text-[#94a3b8]">
