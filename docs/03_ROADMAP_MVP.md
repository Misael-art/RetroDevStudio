...
- O reverse core agora tambem expoe `ExecutionTraceLog`/`CpuState` para trilha dinamica conservadora, filtra analise de codigo por trace opcional sem quebrar API publica e ganhou `BinaryDiffScorer` experimental para matching estrutural entre assembly extraido e compilado.
...
- O `ArtStudio` agora fecha o pipeline basico local ate o build: `Aplicar` cria/atualiza entidade real na cena, o emitter SGDK consome o `SpriteComponent` canonico e `build_orch.rs` trava o `resources.res` para asset vindo do fluxo do ArtStudio.
...
- O reverse core permanece `Experimental`: manifesto, extractors, disassembly inicial e anotacoes ja estao reais e cobertos, mas `trace` Libretro, projecao `.rds` e recuperacao avancada de logica continuam em hardening.
