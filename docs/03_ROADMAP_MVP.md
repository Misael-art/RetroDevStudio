...
**Fase ativa real:** Release candidate / beta testing do desktop Tauri, com RC hotfixado apos validacao manual inicial, onboarding/template/fluxo de autoria endurecidos no editor real, packaging MSI reemitido e updater em placeholder por politica de dependencias
...
- Auditoria residual de UX, com prioridade para revalidar o fluxo de autoria pos-hotfix (cena vazia -> sprite inicial -> inspector -> build) e as superficies ainda `Experimental` (`Asset Browser`, `VRAM Viewer`, `Reverse Explorer`, `Asset Extractor`, `RetroFX`) antes de transformar o release candidate em beta institucional.
