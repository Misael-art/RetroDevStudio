...
**Nota host/QA (2026-05-20, rodada 49):** a branch `codex/sgdk-logic-truth-product-qa` rerodou a barra completa local. `sgdk_corpus_real_build_rom_emulation_report --ignored` fechou **122/122** processados: **68** build/ROM/emulacao visivel, **54** bridge-only, **0** falhas, `fake_toolchain_used=false`. `sgdk_matrix_corpus_ --ignored` passou **7/7** e o no-code oficial SGDK/Libretro passou com ROM real. A evidencia SGDK Logic visual ficou em `qa-rc-2026-05-20T17-49-07-349Z-*`; ela prova UX/diagnostico/source mapping/gaps, nao equivalencia 1:1 de gameplay.

**Nota main/readiness (2026-05-18, rodada 45):** PR #5 (`codex/sgdk-stable-node-engine-blaze`) foi mergeado em `main` por `a31357072c93431ce441ff51a88a51004365ca88` apos checks remotos verdes e `npm run release:readiness:promotion` passou no destino com `Pronto para promocao: SIM`. A matriz tecnica da rodada 44 permanece a evidencia de corpus/SGDK; esta nota registra a governanca de integracao, nao altera sozinha badges/status publico de superficies experimentais.

**Nota host/QA (2026-05-17, rodada 44):** criterio rigoroso preservado: emulacao real exige framebuffer visivel (`emulation_visible_ok`, `non_black_pixels > 0`). `sgdk_corpus_real_build_rom_emulation_report --ignored` fechou **122/122** em `F:\Projects\MegaDrive_DEV\SGDK_Engines`: **68** build/ROM/emulacao visivel, **54** bridge formal, **0** falhas, `stable_candidate=true`, `fake_toolchain_used=false`. Falhas anteriores **Mega Drive Breakout** e **Procedural Animation** (framebuffer preto apos 450 frames) foram resolvidas com `-DRDS_CORPUS_VISIBLE_SMOKE` e supressao honesta de `XGM_startPlay` no modo de smoke. Report: `src-tauri/target-test/validation/sgdk-corpus-real-build/sgdk-corpus-real-build-report.{json,md}`. Regressoes dedicadas para Breakout e Procedural Animation. No-code real (`non_black_pixels=15506`) e BLAZE compat (`non_black_pixels=71680`) rerodados verdes em 2026-05-17.

**Nota host/QA (2026-05-14, rodada 43):** a branch `codex/sgdk-stable-node-engine-blaze` removeu fake da prova Stable. A rodada 43 ainda aceitava emulacao sem pixels visiveis; a rodada 44 corrigiu isso sem rebaixar o criterio.
...
**Ultima revalidacao registrada nesta sessao (2026-05-20, rodada 49, branch `codex/sgdk-real-proof-integration`):** a fatia do extrator passou `npm run check:tree`, `npm run lint`, `npx tsc --noEmit`, `npm test` (**315**), `cargo clippy --manifest-path src-tauri/Cargo.toml -- -D warnings`, `cargo test --manifest-path src-tauri/Cargo.toml --lib -- --nocapture --test-threads=1` (**354 passed / 18 ignored**), `cargo test sgdk_logic ...` (**7 passed / 1 ignored**), `cargo test sgdk_logic_real_corpus_vertical_reports ... --ignored`, `cargo test sgdk_corpus_inventory ...`, `cargo test sgdk_corpus_inventory_real_corpus_report ... --ignored` (**122 projetos**) e `cargo test sgdk_matrix_corpus_ ... --ignored` (**7/7**) com `SGDK_ROOT=F:\Projects\MegaDrive_DEV\sdk\sgdk-2.11` e `RETRODEV_LIBRETRO_CORE_MEGADRIVE=F:\Projects\RetroDevStudio\toolchains\libretro\cores\genesis_plus_gx_libretro.dll`. A primeira tentativa da matriz sem `SGDK_ROOT` falhou corretamente por recusar fake; a segunda, sem core Libretro no worktree isolado, falhou apenas no BLAZE. A reexecucao com toolchain/core reais fechou verde.
**Ultima revalidacao registrada nesta sessao (2026-05-20, rodada 49, branch `codex/sgdk-logic-truth-product-qa`):** a branch passou `npm run check:tree`, `npm run lint`, `npx tsc --noEmit`, `npm test` (**325**), `cargo clippy --manifest-path src-tauri/Cargo.toml -- -D warnings`, `cargo test --manifest-path src-tauri/Cargo.toml --lib -- --nocapture --test-threads=1` (**351 passed / 17 ignored**), `official_sgdk_nocode_game_builds_and_runs_with_real_toolchain --ignored`, `sgdk_corpus_real_build_rom_emulation_report --ignored` (**122/122**, **68** visiveis, **54** bridge-only), `sgdk_matrix_corpus_ --ignored` (**7/7**), `npm run preflight:sgdk-e2e`, `npm run test:e2e:desktop:qa-rc` (`qa-rc-2026-05-20T17-49-07-349Z-*`), `build:debug`, `build:portable`, `build:msi` e `validate-upstream-windows.ps1 -SkipRustTests`. Isto mantem Build/ROM/Emulacao cobertos e SGDK Logic **Parcial/Experimental**.
