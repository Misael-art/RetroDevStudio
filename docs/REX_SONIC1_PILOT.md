...
### Continuação 2026-09-23 — reinserção de tiles com tamanho preservado (Experimental; sem merge)

Operação reutilizável sobre o contrato já comprovado do frame `sonic1_sonic/stand` (sem perfil novo): pixels do frame 32×40 são mapeados pela mesma geometria do compositor (mapping `0x21293`) para nibbles da arte 4bpp **não comprimida** `Art_Sonic 0x21AFE+0xA120` e escritos numa cópia persistida; o tamanho da ROM é preservado por construção. Código 68K confirma a origem: `lea $217FE,a2` (tabela DPLC) e `lea $21AFE,a1` (arte).

Referências e compartilhamento lidos da ROM: tabela DPLC em `0x217FE`, 88 frames; stand = frame 1, entrada `04 2000 7003 200B 200E` (tiles 0..16, coerente com as peças 3/8/3/3 do mapping); **tiles 11..16 também são carregados pelo frame 5**; 0..10 exclusivos.

Recusas explícitas (`edit_*`): pixel sem peça (`edit_pixel_unmapped`), índice > 15, tiles compartilhados sem confirmação (`edit_tile_shared`, lista os frames), recurso não verificado/comprimido (`edit_format_unsupported`), escrita fora do intervalo de arte ou mudança de tamanho (`edit_growth_unsupported`), ROM fora do perfil, no-op. A base é re-hasheada após a edição (`base_rom_sha256_after`).

Evidência:
- Teste técnico `sonic1_stand_tile_reinsertion_observed_in_game` (core oficial): com `obFrame=1` confirmado na RAM (frame stand em tela entre ~1108 e ~1422 com boot+START no 900), recolorir os tiles exclusivos 3..10 alterou 293 pixels, todos numa caixa 25×16; base determinística. Achado: no frame 1500 o Sonic exibe o frame 2 (animação de espera), por isso uma observação ali não prova nada — o teste agora exige `obFrame=1`.
- E2E desktop `inspection-sonic-tiles` (`npm run test:e2e:desktop:inspection:sonic-tiles`), resumo `src-tauri/target-test/validation/sonic1-pilot-2026-09-23T03-37-29-745Z/sonic1-stand-tiles-summary.json`: pela UI, negativo (28,2) recusado; negativo em tiles compartilhados recusado ("Tiles {12} … frames DPLC [5]"); reinserção do retângulo (10,14) 12×8 → índice 14 igual à mutação independente calculada no harness a partir do mapping da ROM; BPS `c48337ef…` aplicado reproduz exatamente a ROM `5e44f9d2…`; base `c7da53a1…` intacta; composição pixel-exata pela UI; no jogo, 96 pixels diferentes da base, caixa (66,164)–(93,179); salvar/reiniciar/reabrir restaura a edição.

Limites: somente o frame stand verificado; sem codecs (Nemesis/Kosinski etc. recusados por não serem o recurso verificado); sem crescimento; compartilhamento só é conhecido via DPLC do Sonic; nenhuma extração universal.
