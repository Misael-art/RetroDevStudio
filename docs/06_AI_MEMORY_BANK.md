...
**Ultima Atualizacao:** 2026-05-11 (rodada 32 - host Windows rechecado, gates tecnicos verdes, QA/corpus/packaging frescos; promocao bloqueada por governanca Git)
...
- **Git/governanca:** worktree versionavel limpo antes das atualizacoes documentais; branch rastreia `origin/feat/sgdk-vram-residency-streaming-r14`; `origin/main...HEAD = 0/201`. `Rascunho.txt` existe na raiz, mas esta ignorado em `.git/info/exclude` e nao deve ser versionado sem curadoria.
...
- **Readiness estrito:** `npm run release:readiness:promotion` rodou baseline, upstream e Desktop E2E simples, consumiu `manual-qa-status.json` fresco e saiu com codigo 1 por um unico bloqueio: `Promocao para a trilha publica ainda pendente: feat/sgdk-vram-residency-streaming-r14 esta 201 commit(s) a frente de origin/main.`
