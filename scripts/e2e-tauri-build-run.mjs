...
async function runReferencePlatformerScenario(sessionId, timeoutMs, onProjectCreated) {
  ...
  // Audio layers, kept separate: (1) generated by the core is proven by the Rust contract
  // test; (2) forwarded to the WebAudio output graph is read from the app's telemetry;
  // (3) acoustic/loopback is a best-effort capture of the default sink monitor.
  const audioBefore = await executeScript(sessionId, "return window.__RDS_E2E__?.getAudioOutputTelemetry?.() ?? null;");
  await new Promise((resolve) => setTimeout(resolve, 1500));
  const audioAfter = await executeScript(sessionId, "return window.__RDS_E2E__?.getAudioOutputTelemetry?.() ?? null;");
  ...
  if (!forwarded || forwarded.receivedNonZeroFrames <= 0 || forwarded.renderedNonZeroFrames <= 0 || forwarded.contextState !== "running") {
    fail(`Audio do core nao foi encaminhado ao AudioContext em execucao: ${JSON.stringify({ audioBefore, audioAfter })}`);
  }
