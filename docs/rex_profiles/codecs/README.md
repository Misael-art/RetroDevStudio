...
| lz4w-sgdk | OK | OK (12 plains RT-OK + 9 goldens GOLDEN-CONFIRMED + dicionário d01 DICT-RT-OK com paridade de espelho byte a byte, manifest com hashes) | pendente | OK 7 negativos negative-spec derivados do contrato (truncated ×3, invalid-reference ×3, excessive-output ×1; oráculos não executados neles); defeitos do oráculo registrados | N/A estrutural: sem 2º oráculo multi-autor (limitação declarada no manifest); **prova 68k executada 2026-09-25** (MAME 0.289 + desempacotador byte-idêntico à libmd.a): saídas IDÊNTICAS jar↔68k nas streams válidas do encoder com bit-fonte ROM (8 casos discriminantes; evidência `variants-68k-2026-09-25`); exceção em casos de fronteira inválidos = diagnóstico da referência; **replay 2026-09-26 das streams do ENCODER RUST (10 casos, comparação tripartite completa 68k↔jar↔esperado)**: divergência real encontrada e reduzida ao mínimo (referência longa não-ROM com off ≥ 16386 words vira leitura PARA FRENTE no unpacker 68k — aritmética de 16 bits em `tools_a.s`; fronteira off=16385 OK; WIP do integrador com janela 0x4000 decodifica idêntica no hardware) — evidência `rust-streams-2026-09-26` | Experimental (vetores+negativos+dicionário confirmados por oráculo único do ecossistema + desempate 68k medido; produto aguarda contrato) |
| nemesis | OK (mdcomp HEAD; LGPL, ferramenta externa) | parcial: 9 plains RT-OK no domínio ×32 (hashes no manifest); goldens literais **blocked com motivo** (exigem espelhar tabela adaptativa de nibble) | pendente | OK 7 negativos negative-spec (n01–n07) derivados do contrato, cada um com **sonda determinística re-asserida no build** (padding 6→32/100→128; lixo 64B→1048544B exatos; truncado com tamanho pleno e conteúdo divergente; prefixo mata o oráculo no sandbox rc=137; stream real + max_out); domínio ×32 e >0 explícitos; encode vazio SEGFAULTA a referência | blocked: 1 oráculo só nesta rodada | Experimental (roundtrip+negativos fixados; produto aguarda contrato) |
...
| enigma | OK (mdcomp HEAD; LGPL, ferramenta externa) | parcial: 10 arrays int16 BE RT-OK (hashes no manifest); goldens literais **blocked com motivo** | pendente (semântica dos campos de header após o byte de modo não fixada nesta fase — registrada como pendência de fase 3) | OK 7 negativos negative-spec (e01–e07) com **sondas determinísticas re-asseridas no build**: ímpar 5→4 (cauda descartada — produto DEVE recusar, diretriz do integrador), truncado −1→4098≠4096, declaração inflada ignorada byte-a-byte, modo 0x02 inexistente decodificado como válido, vazia/lixo aceitas sem erro, stream real + max_out | blocked: 1 oráculo só nesta rodada | Experimental (roundtrip+negativos fixados; produto aguarda contrato) |
