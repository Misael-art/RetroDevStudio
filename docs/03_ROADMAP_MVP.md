...
**Nota 2026-05-19 (rodada 47, branch `codex/command-dat-artstudio-node-runtime`):** implementada e validada uma frente paralela isolada para `command.dat -> comandos visuais -> nodes executaveis`, sem tocar na worktree GameMaker vertical. A entrega adiciona parser local/BYOR em Rust e frontend para `[Command]`, `name`, `command`, `time`, numpad `_1.._9`, aliases direcionais, `_P/_K`, botoes MUGEN `a,b,c,x,y,z`, simultaneo `+`, sequencia por virgula e janela de frames. `SpriteComponent.commands` passa a persistir bindings experimentais; ArtStudio exibe painel `Comandos`, importa `command.dat`, mostra chips por perfil Mega Drive/SNES/teclado/mouse, associa comandos a sequencias e cria sequencias a partir deles. NodeGraph ganhou `input_command`, quick action de comando de luta e codegen SGDK/SNES com matcher por ring buffer no subset suportado. Tokens fora do subset entram em `unsupported_tokens` e bloqueiam C gerado com `#error` acionavel. A prova real `official_sgdk_nocode_game_builds_and_runs_with_real_toolchain --ignored` passou com SGDK v2.11/Libretro reais, ROM real e framebuffer visivel para projeto no-code com Hadouken por `input_command`; QA RC A-G e `build:debug` tambem passaram. Status: **Experimental**; sem scraping de wikis comunitarias, sem claim de compatibilidade total com todo dialeto de `command.dat` e sem promocao de Node Engine/ArtStudio antes de QA ampla com casos reais variados.
...
| `gamemaker`      | Fora do MVP/Q2 | Experimental  | Local        | Registry `importable: true`; adapter GMX/GMZ/GMEZ importa sprites/objetos/rooms e preserva GML como bridge semantica editavel. Prova host-local cobre `F:\Projects\Game Maker\Basic_platform_game_example.gmez` e GMX em `F:\Projects\Godot\Super Bowsette Source Code Beta 0.1\src`. | Falta conversao GML nativa completa, build SGDK/ROM/emulacao institucional e relatorio de equivalencia origem->nodes->SGDK. | Nao                           |
