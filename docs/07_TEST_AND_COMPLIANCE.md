...
- Mudancas que adicionem ou alterem candidates de core Mega Drive devem cobrir `DependencyKind::LibretroMegaDriveCore` e `CoreTarget::MegaDrive` para todos os nomes suportados (`genesis_plus_gx_libretro`, `picodrive_libretro`, `blastem_libretro`). A presenca de BlastEm no upstream oficial nao substitui validacao local com o artefato real do core instalado na extensao canonica do host (`blastem_libretro.dll` no Windows, `.so` no Linux, `.dylib` no macOS); se o core nao existir no host, a sessao deve registrar o bloqueio e pode validar a matriz cross-core com outros cores reais disponiveis sem declarar BlastEm executado.
