...
| Troca `frame-0 → frame-1 → frame-0` | runtime `5b16970`; binário `ee3aac…` | mesma ROM; sessão `inspection-1789869813-00000000` | passou; `label=prévia anterior removida na troca` e `previousPreviewRemovedOnChange=true` nas duas transições; este E2E não é a prova de resposta fora de ordem | log E2E da execução final |
...
| Resposta fora de ordem A→B→A | `f0d3347`; UI `InspectionPanel.test.tsx:335` | promises controladas; A pendente, B resolvido, A liberado por último | passou; imagem, seleção `frame-1` e `metadata_source` B permanecem; A não reaparece | `npm test -- ... InspectionPanel.test.tsx` (8/8) |
| Negativos de frame desconhecido/metadado incorreto | `f0d3347`; testes de UI/Rust | manifests controlados sem frame ou com frame/manifesto divergente | rejeitados; a composição não reutiliza a prévia anterior | testes `sprite_composition` e painel de inspeção (8/8) |
