...
- Mudancas na camada de Gameplay Parity (`parity_harness.rs`, `parity_run_capture`, `parityService.ts`, eixo `gameplay_parity`, UI em `ProjectCapabilityPanel`/`ToolsPanel`) devem cobrir: comparacao por hash SHA-256 de framebuffer por frame + estado final, deteccao de divergencias (rom mismatch, frame count, framebuffer hash, non-black pixels, final state), replay de golden inputs e rejeicao de `golden_path` vazio. A superficie canonica de UI e a aba **Tools -> Parity Capture** (nao existe `RuntimeConsolePanel`); `ProjectCapabilityPanel` apenas exibe o ultimo relatorio e orienta para a aba, sem botao que dispare captura com golden vazio. Limites honestos obrigatorios: o harness **nao e BlastEm, nao faz cycle accuracy e nao prova equivalencia de gameplay 1:1**; `not_measured_by_this_harness` deve continuar listando `audio_exact_match`, `m68k_cycle_trace`, `z80_cycle_trace`, `vdp_scanline_trace`, `dma_timing`. Gameplay Parity permanece `Experimental` ate haver harness de equivalencia dedicado (W7.4 BlastEm / W7.5 cycle report).
