...
**Ultima revisao canonica:** 2026-06-17 (rodada 71 - W7.3 finalizada com gates verdes e artefatos frescos)
...
| Surface Registry (governanca de superficies) | Experimental | Em codigo | Local | Fatia 1 UI: `src/core/surfaceRegistry.ts` deriva o rail com `minPersona`/`maturity`/`capability`/`roadmapRef`; `surfaceRegistry.test.ts` falha se a linha correspondente sumir desta matriz (gate registry x roadmap) | Estender enforcement para Tools tabs e importadores; regra normativa no roadmap depende de decisao humana (estudo de UI, secao 12) | Nao                           |
| Seletor de persona do shell            | Experimental | Em codigo     | Local         | Fatia 1 UI: seletor Guiado/Criador/Pro/Hacker no topbar com default `pro` (zero regressao), gating do rail/comandos por `minPersona`, persistencia localStorage e sugestao local por metricas | Falta prova institucional (`qa-rc`) e calibracao dos gatilhos de transicao em uso real  | Nao                           |
| Shell adaptativo por densidade         | Experimental | Em codigo     | Local         | Fatia 1 UI: `data-density` no shell, corte compacto 1440 (Deck 1280/notebook 1366), layout salvo por perfil de densidade com fallback legado; cobertura em `workspaceLayout.test.ts`        | Falta bloco H estendido com asserts por resolucao e drawers overlay no perfil compacto  | Nao                           |
| Autosave de rascunho local             | Experimental | Em codigo     | Local         | Fatia 1 UI: draft de cena em localStorage com debounce, badge no topbar e banner de recuperacao que restaura no editor sem gravar em disco; disco so muda no fluxo canonico de save         | Falta prova institucional de recuperacao pos-crash real                                 | Nao                           |
| Metricas locais de produto             | Experimental | Em codigo     | Local         | Fatia 1 UI: contadores 100% locais (builds ok/fail, budget blocks, visitas, TTFR) sem rede, opt-out via storage; alimentam sugestao de persona; cobertura em `productMetrics.test.ts`       | Falta UI de opt-out e validacao dos gatilhos em uso real                                | Nao                           |
