...
**Ultima revisao canonica:** 2026-07-10 (rodada 78 — codex/desktop-e2e-determinism-p0)

**Nota 2026-07-10 (rodada 84, branch `codex/desktop-e2e-determinism-p0`):** Desktop E2E P0 — Em hardening; corrida de hidratacao tardia do HierarchyPanel identificada e corrigida localmente; nao certificado. Causa raiz: `HierarchyPanel.useEffect` iniciava async `listScenes/getSceneData/hydrateSceneResult`; E2E injetava `setSceneDraft` (rev=N); hidratacao tardia chamava `setActiveScene` gerando rev=N+1 e sobrescrevendo draft. Correcao: guard por revisao no `HierarchyPanel` (captura `sceneRevision` no inicio do efeito, compara apos cada IPC, nao sobrescreve se mudou). Bridge canonica: `isLiveValidationFreshMatchingRevision` exposta via `window.__RDS_E2E__`; runner MJS usa a bridge em vez de logica duplicada. HTTP 403 e `assetProtocol.scope` nao alterados — sem relacao causal comprovada. Testes de regressao deterministicos (Promise adiada) provam: hidratacao tardia nao sobrescreve draft; hidratacao inicial normal funciona; troca de projeto ignora resposta tardia. Gates locais: check:tree, lint, tsc --noEmit, npm test, clippy -D warnings, cargo test --lib, build:debug. CI remoto pendente.
