...
- O wizard de primeiro uso agora prioriza por padrao um template builtin realmente `ready-to-create` neste host, exibe um card `Primeiro sucesso` com o caminho `Scene -> Game`, manteve os templates SGDK externos honestamente bloqueados ate donor manual e passou a tratar `Importar Externo` como trilha secundaria expansivel em vez de competir com o caminho principal no footer.
...
- Nesta sessao (`2026-04-10`), o host local voltou a bloquear o binario `tauri-driver.exe` por policy/AppLocker; portanto, a repeticao institucional de `desktop E2E` e `qa-rc` desta rodada precisa ocorrer no workflow GitHub/Windows apos o push, mesmo com `build:debug` e `validate-upstream-windows` novamente verdes localmente.
...
**Status de execucao em 2026-04-09:** frentes `1-4` seguem fechadas e verificadas no estado atual do branch; a frente `5` continua em hardening, mas o pacote `onboarding / primeiro sucesso` acabou de ganhar prova desktop institucional nesta wave. `src/App.tsx` passou a manter o wizard com corpo rolavel e footer fixo, `Criar Projeto` agora respeita `readyToCreate`, e `scripts/e2e-tauri-build-run.mjs` foi alinhado ao template builtin `starter_guided` e aos affordances reais do shell atual. Nesta mesma rodada passaram `onboarding-shell`, `qa-rc`, `validate-upstream-windows` e `release:readiness:baseline`, sem abrir frente nova nem alterar o fluxo canonico `Build -> ROM -> Emulacao`. O build atual continua medindo o chunk principal em ~`338.02 kB` bruto / `102.78 kB` gzip, `ViewportPanel` em ~`53.59 kB` bruto / `16.60 kB` gzip, `ToolsPanel` em ~`67.13 kB` bruto / `17.35 kB` gzip e `ReverseWorkspace` em ~`26.16 kB` bruto / `5.81 kB` gzip. O proximo alvo real do trimestre volta a ser `densidade do shell`, ainda sem abrir docking livre nem promover superficies `Experimental`.
