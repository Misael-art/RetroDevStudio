...
| Build funcional (import -> salvar -> reabrir -> ROM funcional) | E | Parcial, Local (2026-04-19 rodada 8) | Rodada 7 + preflight de host (`scripts/sgdk-e2e-host-preflight.mjs`, `npm run preflight:sgdk-e2e`, log no arranque do runner E2E); fixture `sgdk_e2e_donor` com `main.c` + `player_control.*` para mesmo scan multificheiro que testes Rust; sucesso completo continua a depender de toolchain, `tauri-driver` e WebDriver no host |
...
- **Fase E - Build funcional. (parcial; preflight host em 2026-04-19 rodada 8.)** Rodada 7 + preflight explicito de `toolchains/sgdk`, `tauri-driver` e msedgedriver antes do fluxo E2E; fixture `sgdk_e2e_donor` alinhada ao scan multi-ficheiro; QA RC **A–G** inalterado em ambito; sucesso E2E continua a exigir dependencias reais no host.
