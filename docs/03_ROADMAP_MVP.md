...
**Nota 2026-07-10 (rodada 78, branch `codex/desktop-e2e-determinism-p0`):** Bloqueadores P0 do fluxo `Build -> ROM -> Emulacao` resolvidos. Contrato de revisao causal endurecido: `setSceneDraft` devolve receipt com `sceneRevision` obrigatorio; `waitForLiveValidationFresh` reestruturado com 4 casos exclusivos (nao-fresh, fresh+rev menor, fresh+rev maior/pulo, fresh+rev exata), sem condicao inalcancavel. Predicado extraido como `isLiveValidationFreshMatchingRevision` puro em `liveValidationController.ts`, testado via helper compartilhado (16 testes). Cenario live-error exige `hwValidatedRevision === expectedRevision`. Receipt invalido falha imediatamente sem `?? 0`. HTTP 403 do Viewport fetch de asset separado do rate limit de dependency_manager.rs; scope do protocolo de asset ampliado com `/tmp/**`. Gates: check:tree, lint, tsc --noEmit, **47/1 test files**, clippy -D warnings, **438/0/24 cargo test --lib**, build:debug. CI remoto pendente.
