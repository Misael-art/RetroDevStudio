...
| 7 | `BLAZE_ENGINE [VER.001] [SGDK 211] [GEN] [ENGINE] [BRIGA DE RUA]` | Beat 'em up (estresse) | Passou | Passou | Passou | Passou | Passou | Parcial | Passou | Passou | **Passou por compat** | **Parcial** | Evidencia rodada 42 (2026-05-14): `sgdk_matrix_corpus_blaze_engine_partial_flow_documents_build_blocker --ignored` passou. O teste confirma que o original ainda apresenta blocker real de hardware antes da transformacao, depois exige build com `SGDK compatibility profile`, `sprite culling`, `multiplex`, breakdown `MD VRAM analysis` com `spr_res=`/`banks=` e ROM com assinatura `SEGA`. Mantem **Parcial** porque e perfil transformado/compatibilidade, nao equivalencia plena do original nem emulacao final por projeto. |
...
- **Rodada 42 (2026-05-14):** `BLAZE_ENGINE` saiu de falha legitima sem build para perfil conservador de compatibilidade com build/ROM fake `SEGA`; o corpus inventory real passou com **122 projetos**, **2.511 gaps** e **0 blockers** por bridge formal. SGDK segue **Experimental** ate existir build/ROM/emulacao individual para os 122 e gates completos na mesma revisao.
