...
**Regra de produto:** SGDK permanece **Experimental** ate os seis titulos-base abaixo passarem no fluxo completo (import -> editar -> salvar -> reabrir -> build/ROM) com evidencia documentada e sem regressao nos gates do repositorio.
...
**Cobertura por genero (codigo):** existem **sete** testes Rust `#[ignore]` em `src-tauri/src/core/project_mgr.rs` (seis titulos-base + um corpus de estresse `BLAZE_ENGINE`). Os seis titulos-base mantem o contrato completo com assert de ROM `SEGA`; `BLAZE_ENGINE` e executado como prova de **blocker legitimo auditavel** (nao faz assert de ROM). Ausencia do doador com `--ignored` => **panic**, salvo `RDS_SGDK_MATRIX_CORPUS_SKIP=1`. Isto nao promove `support_status`.
...
**Ultima revalidacao registrada nesta sessao (2026-05-11, rodada 36, branch `codex/product-hardening-runtime-setup`):** `npm run check:tree` OK, `npm run lint` OK, `npx tsc --noEmit` OK, `npm test` (**292** passed), `cargo clippy --manifest-path src-tauri/Cargo.toml -- -D warnings` OK, `cargo test --manifest-path src-tauri/Cargo.toml --lib -- --nocapture --test-threads=1` (**329** passed / **10** ignored), `cargo test sgdk_matrix_corpus_ --manifest-path src-tauri/Cargo.toml --lib -- --ignored --nocapture --test-threads=1` (**7** passed), `powershell -NoProfile -ExecutionPolicy Bypass -File scripts\\validate-upstream-windows.ps1 -SkipRustTests` (**exit 0**, `success=true`), `npm run preflight:sgdk-e2e` (**Ready: SIM**), `npm run test:e2e:desktop:qa-rc` (**OK**, evidencias `qa-rc-2026-05-11T23-49-20-465Z-*`), `npm run build:debug`, `npm run build:portable` e `npm run build:msi` (**OK**). SGDK segue **Experimental**; sem promocao de `support_status`.
...
| 7 | `BLAZE_ENGINE [VER.001] [SGDK 211] [GEN] [ENGINE] [BRIGA DE RUA]` | Beat 'em up (estresse) | Passou | Passou | Passou | Passou | Passou | Parcial | Passou | Passou | **Falhou** | **Falhou (legitimo)** | Evidencia rodada 14 (2026-04-23): `MATRIX_BLAZE hw`: `mode=sgdk_managed`, `total_kb=6221`, `resident_kb=37`, `streamable_kb=6183`, `dma_frame_kb=1559`, `fatal=1`, `warn=4`. Build bloqueado por restricao fatal explicita (ex.: `Sprite overflow` e/ou `VRAM Overflow (resident)` conforme import). Rodada 15+: stdout `MATRIX_BLAZE hw` inclui `spr_res_kb`, `tile_kb`, `hud_kb`, `strm_spr_kb`, `anim_sw_kb`, `banks`, `cells`; log de build `MD VRAM analysis` inclui `spr_res=` e `banks=` para auditoria de heuristica. |
