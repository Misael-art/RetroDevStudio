...
* **Proximo passo imediato:**
  1. Reproduzir o caminho canonico de build desktop local (`scripts/build.mjs debug` / `npm run build:debug`) neste host sem override manual de `beforeBuildCommand`, para fechar o ultimo ponto de diferenca entre o smoke local e o pipeline de packaging esperado.
  ...
  3. Retomar a trilha do reverse core por ondas pequenas (`trace` com Libretro real, fixtures sinteticas de texto/audio/grafico e cobertura incremental do disassembler) sem poluir a arquitetura nem abrir claims maiores do que a prova sustentada.
