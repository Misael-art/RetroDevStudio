...
**Ultima revalidacao registrada nesta sessao (2026-05-11, rodada 32, branch `feat/sgdk-vram-residency-streaming-r14`):** `npm run check:tree` OK, `npm run lint` OK, `npx tsc --noEmit` OK, `npm test` (**290** passed), `cargo clippy --manifest-path src-tauri/Cargo.toml -- -D warnings` OK, `cargo test --manifest-path src-tauri/Cargo.toml --lib -- --nocapture --test-threads=1` (**325** passed / **10** ignored), `cargo test sgdk_matrix_corpus_ --manifest-path src-tauri/Cargo.toml --lib -- --ignored --nocapture --test-threads=1` (**7** passed), `powershell -NoProfile -ExecutionPolicy Bypass -File scripts\\validate-upstream-windows.ps1 -SkipRustTests` (**exit 0**, `success=true`), `npm run preflight:sgdk-e2e` (**Ready: SIM**), `npm run test:e2e:desktop:qa-rc` (**OK**, evidencias `qa-rc-2026-05-11T11-53-47-951Z-*`). SGDK segue **Experimental**; sem promocao de `support_status`.
