// Testes do perfil snes-hirom contra expectativas pinadas ANTES da
// implementacao (commit das expectativas: 08f0f87; ver
...
import { translate, invert, read, validateState } from './snes-hirom.mjs';
import { buildSnesHiromFixture, sha256 } from './build-fixtures.mjs';

const HERE = dirname(fileURLToPath(import.meta.url));
const EXPECTED_PATH = join(HERE, '../../../data/rex_profiles/addressing/snes-hirom/expected/translate-cases.json');
...
test('fixture autoral reproduz o sha256 pinado', () => {
  const rom = buildSnesHiromFixture();
  assert.equal(rom.length, expected.fixture.rom_size);
  assert.equal(sha256(rom), expected.fixture.sha256);
});

test('translate: casos positivos pinados (offset linear 24 bits, sobreposicao 00x/40x, regioes)', () => {
...
test('translate: negativos pinados (reservados unsupported; sem ambiguous em HiROM padrao)', () => {
...
test('translate discrimina formula: HiROM e linear (banco<<16)+a, nao (banco&0x7F)*0x8000 de LoROM', () => {
  // LoROM daria 0x000000 para 0x008000; HiROM da 0x008000.
  assert.deepEqual(translate(0x008000, { rom_size: 0x100000 }), { region: 'rom', offset: 0x008000 });
  // meio do banco 41: linear 0x11234; qualquer formula por pagina de 32KB erra.
  assert.deepEqual(translate(0x411234, { rom_size: 0x100000 }), { region: 'rom', offset: 0x011234 });
  // alias 80 mascarado (rom de 1MB)
  assert.deepEqual(translate(0x808000, { rom_size: 0x100000 }), { region: 'rom', offset: 0x008000 });
  // em 4MB o banco 80 NAO e alias de 00 (mascara maior que o alias A15 sozinho)
  assert.deepEqual(translate(0x808000, { rom_size: 0x400000 }), { region: 'rom', offset: 0x008000 });
  assert.deepEqual(translate(0xC11234, { rom_size: 0x400000 }), { region: 'rom', offset: 0x011234 });
});
...
test('read: segmentos pinados (corrida contigua entre bancos completos, corte em grupo/espejo/espacos)', () => {
  const rom = buildSnesHiromFixture();
...
test('evidencia: hash do arquivo de expectativas bate com o registrado na evidencia', () => {
  const evidencePath = join(HERE, '../../../data/rex_profiles/addressing/snes-hirom/evidence/translate-fixture.json');
