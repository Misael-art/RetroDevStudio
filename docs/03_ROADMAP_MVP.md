...
**Status de execucao em 2026-04-09:** frentes `1-4` seguem fechadas e verificadas no estado atual do branch; a frente `5` continua em hardening com tres entregas conservadoras no shell: `ViewportPanel` saiu do chunk inicial, o `WorkspaceGuideCard` passou a priorizar so as acoes principais no topo e o wizard de primeiro uso moveu `Importar Externo` para uma trilha secundaria expansivel para proteger o caminho principal `criar/abrir projeto`. O build atual mede o chunk principal em ~`337.78 kB` bruto / `102.72 kB` gzip, `ViewportPanel` em ~`53.59 kB` bruto / `16.60 kB` gzip, `ToolsPanel` em ~`67.13 kB` bruto / `17.35 kB` gzip e `ReverseWorkspace` em ~`26.16 kB` bruto / `5.81 kB` gzip. A baseline do Vitest voltou a ficar repetivel neste host com `pool: "threads"` + worker unico; `Performance do shell` segue materialmente melhor que antes, mas o proximo alvo real do trimestre continua sendo terminar `onboarding / primeiro sucesso`, ainda sem abrir docking livre nem novas superficies.
