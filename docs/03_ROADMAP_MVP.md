...
**Ultima revisao canonica:** 2026-06-28 (rodada 77 - W7.4/W7.5 cross-core + cycle evidence em hardening)

**Nota 2026-06-28 (rodada 77, branch `codex/w7-4-blastem-parity`):** W7.4/W7.5 avancaram como hardening **Experimental** da camada Gameplay Parity, sem promocao Stable e sem claim de equivalencia gameplay 1:1. W7.4 adiciona Cross-Core Parity generico para dois cores Libretro reais, com relatorio JSON/Markdown, validacao de caminhos e preservacao de divergencia de estado final mesmo quando hashes de framebuffer divergem. `blastem_libretro` passou a ser candidato Mega Drive oficial no dependency manager/autodetect; o upstream Libretro foi verificado, mas este host nao tinha `blastem_libretro.dll`, entao a validacao local real usou `Genesis Plus GX v1.7.4 46a5521` vs `PicoDrive 2.05-046e5ff`. W7.5 adiciona Cycle Report `rds-cycle-report/v1`; como os cores Libretro disponiveis nao expoem traces internos, o report marca `m68k_cycle_trace`, `z80_cycle_trace`, `vdp_scanline_trace` e `dma_timing` como `missing` e `not_cycle_accurate=true`, registrando apenas timings de frame no host, hashes e estado final. UI: novas abas **Tools -> Cross-Core Parity** e **Tools -> Cycle Report**, ambas `Experimental`. Evidencia local: teste ignorado `w7_4_w7_5_real_cores_generate_reports_when_available` gerou `cross-core-parity-report.{json,md}` e `cycle-report.{json,md}` em `src-tauri/target-test/validation/w7-4-w7-5-real-parity/project/.rds/reports/`. Gates completos passaram: `check:tree`, lint, tsc, `npm test` **442**, `cargo check`, clippy, `cargo test --lib` **457/0/24**, preflight SGDK, `build:debug` e `qa-rc` **A-H**. Proximo passo de roadmap: provisionar BlastEm real e repetir a matriz cross-core com ele, mantendo a superficie experimental ate haver criterio institucional de equivalencia.
